...
exports.login = async (req, res) => {
    ...
    try {
        ...
        if(!validatePassword) return res.status(400).json({ success: false, message: "Password is not correct"})

        const token = generateToken(user.id, user.role);
        if(!token) return res.status(400).json({ success: false, message: "Invalid Token"});

        // return res.header("auth-token", token).status(201).json({success:true, message: "You are now logged in", data: token})
        return res.header("auth-token", token).status(201).json({success:true, message: "You are now logged in", token: token})
    } catch (error) {
        console.log({ message: error.message });
    }
}